...
export default function Login() {
  ...
  const returnUrl =
    ...
    !rawReturn.startsWith("/register")
      ? decodeURIComponent(rawReturn)
      : "/dashboard";
  const { isAuthenticated, user, loginWithPassword, isLoading } = useAuth();
      ...
      const roleDashboard =
        user.role === "mentee"
          ...
          : user.role === "mentor"
            ...
            : "/admin/dashboard";
      const destination =
        !returnUrl || returnUrl === "/dashboard" ? roleDashboard : returnUrl;
      navigate(destination.startsWith("/") ? destination : roleDashboard, {
        replace: true,
      });
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Redirect is handled by useEffect when user state updates (role-based dashboard)
    } catch (err: unknown) {
      const message =
